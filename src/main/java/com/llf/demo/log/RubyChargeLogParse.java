 ...
 */
public class RubyChargeLogParse {

    private static final Map<Long, Long> AMOUNT_MAP = new HashMap<>();
    ...
    public static void main(String[] args) {
        try (
            ...
            for (Map.Entry<Long, Long> entry : AMOUNT_MAP.entrySet()) {
                Map<String, Long> map = new HashMap<>();
                map.put("uid", entry.getKey());
                map.put("amount", entry.getValue());
                map.put("count", countJson.getLongValue(entry.getKey().toString()));
                list.add(map);
            }

            list = list.stream().filter(m -> {
                Long amount = m.get("amount");
                Long count = m.get("count");
                if (amount < 10000){
                    return false;
                }
                return amount >= 100000 && count < 10;
            }).sorted((o1, o2) -> {
                Long amount1 = o1.get("amount");
                Long amount2 = o2.get("amount");
            ...
            File file = new File("C:\\Users\\lilingfeng\\Downloads\\battleLog\\uidAmount14.xls");
            FileOutputStream fileOutputStream = new FileOutputStream(file);
            String[] titles = {"uid", "amount", "count"};
            ExcelUtil.exportFile(list, titles, titles, fileOutputStream);
        } catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private static void handleLine(String line){
        ...
        Long totalAmount = AMOUNT_MAP.getOrDefault(uid, 0L);
        AMOUNT_MAP.put(uid, totalAmount + amount);
    }

    public static String parseLog(String log){
        int startIndex = log.indexOf("handleRubyRecharge, msg:");
        if (startIndex != -1) {
            startIndex += "handleRubyRecharge, msg:".length();
            int endIndex = log.indexOf("}\"}", startIndex);

            if (endIndex != -1) {
                return log.substring(startIndex, endIndex + 1);
            } else {
                System.out.println("JSON message not found.");
            }
        } else {
            System.out.println("JSON message not found.");
        }
